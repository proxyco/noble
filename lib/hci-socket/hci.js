...
Hci.prototype.init = function () {
  ...
  if (process.env.HCI_CHANNEL_USER) {
    console.log('hci.init bindUser')
    this._deviceId = this._socket.bindUser(deviceId);
    this._socket.start();

    this.reset();
  } else {
    console.log('hci.init bindRaw, deviceId=', deviceId)
...
Hci.prototype.setScanParameters = function () {
  ...
  console.log('hci.hci.setScanParams')
  // header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  ...
  cmd.writeUInt8(0x01, 9); // own address type: 0 -> public, 1 -> random
...
Hci.prototype.setScanEnabled = function (enabled, filterDuplicates) {
  const cmd = Buffer.alloc(6);
  console.log('hci.hci.setScanEnabled')
  // header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
...
Hci.prototype.createLeConn = function (address, addressType) {
  ...
  cmd.writeUInt8(0x01, 16); // own address type

  cmd.writeUInt16LE(0x0006, 17); // min interval
  cmd.writeUInt16LE(0x0006, 19); // max interval
  cmd.writeUInt16LE(0x0001, 21); // latency
  cmd.writeUInt16LE(0x0500, 23); // supervision timeout
...
Hci.prototype.writeAclDataPkt = async function (handle, cid, data) {
  ...
  var index = 0
  var pl
//  console.log(`writeAclDataPkt, write ${data.toString('hex')}`)
 
//  for (var aclPktIndex = 0; aclPktIndex < data.length;) {
  while (bytesRemaining > 0) {
//    console.log('writeAclDataPkt bytesRemaining ', bytesRemaining)

    // header
    if (bytesRemaining == data.length) {
      pktDataLenMax = aclMtu - 9
      pktDataLen = bytesRemaining > pktDataLenMax ? pktDataLenMax : bytesRemaining
      //pktDataLen = pktDataLen - 4
      bytesRemaining = bytesRemaining - pktDataLen
      var dlen = pktDataLen + 4
      pkt = Buffer.alloc(9 + pktDataLen);
      pkt.writeUInt16LE(handle | ACL_START_NO_FLUSH << 12, 1);
      pkt.writeUInt16LE(dlen, 3); // data length 1
      pkt.writeUInt16LE(data.length, 5); // data length 2
      pkt.writeUInt16LE(cid, 7);

      pl = Buffer.alloc(pktDataLen)
      data.copy(pl, 0, index, index + pktDataLen)
      index += pktDataLen
//      console.log(`write acl data pkt - bf pl.copy: ${pkt.toString('hex')}`);
      pl.copy(pkt, 9);
//      console.log('pktDataLen=' + pktDataLen + ', bytesRemaining=' + ', dlen=' + dlen
//        + ', data.len=' + data.length + ', index=' + index + ', pl.len=' + pl.length + ', pkt.len=' + pkt.length)
//      console.log(`write acl data pl - writing:                    ${pl.toString('hex')}`);
    } else {
      pktDataLenMax = aclMtu
      pktDataLen = bytesRemaining > pktDataLenMax ? pktDataLenMax : bytesRemaining
      bytesRemaining = bytesRemaining - pktDataLen

      var dlen = pktDataLen   
      pkt = Buffer.alloc(5 + pktDataLen);
      pkt.writeUInt16LE(handle | ACL_CONT << 12, 1);
      pkt.writeUInt16LE(dlen, 3);
...
//    console.log(`write acl data pkt - writing: ${pkt.toString('hex')}`);
    this._socket.write(pkt);
    await new Promise(done => setTimeout(done, 3));
  }
};

Hci.prototype.writeAclDataPkt2 = function (handle, cid, data) {
...
Hci.prototype.onSocketData = function (data) {
  ...
  if (HCI_EVENT_PKT === eventType) {
    ...
    if (cmd === LE_SET_SCAN_ENABLE_CMD) {
      ...
      console.log('\t\t\tLE enable scan command');;
...
Hci.prototype.processCmdCompleteEvent = function (cmd, status, result) {
  if (cmd === RESET_CMD) {
    ...
    if (hciVer < 0x06) {
      ...
      console.log('hci.hci.processCmdCompleteEvent, setScanX 0')
