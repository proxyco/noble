...
Gatt.prototype._queueCommand = function (buffer, callback, writeCallback) {
  //console.log('hci.gatt._queueCommand 0')
  ...
  //console.log('hci.gatt._queueCommand 1, currCmd null')

    while (this._commandQueue.length) {
      //console.log('hci.gatt._queueCommand 2, cmdQ.len=' + this._commandQueue.length)
      this._currentCommand = this._commandQueue.shift();

      this.writeAtt(this._currentCommand.buffer);

      if (this._currentCommand.callback) {
        //console.log('hci.gatt._queueCommand 3')
        break;
      } else if (this._currentCommand.writeCallback) {
        //console.log('hci.gatt._queueCommand 4')
        this._currentCommand.writeCallback();
...
Gatt.prototype.write = function (serviceUuid, characteristicUuid, data, withoutResponse) {
  //console.log('hci.gatt.write 0')
  const characteristic = this._characteristics[serviceUuid][characteristicUuid];

  if (withoutResponse) {
    //console.log('hci.gatt.write 1')
    ...
    //console.log('hci.gatt.write 2')
    this._queueCommand(this.writeRequest(characteristic.valueHandle, data, false), data => {
      const opcode = data[0];

      //console.log('hci.gatt.write callback, opcode = ' + opcode)
      if (opcode === ATT_OP_WRITE_RESP) {
        this.emit('write', this._address, serviceUuid, characteristicUuid, 0);
      } else {
        if (opcode === ATT_OP_ERROR && data.length == 5) {
          this.emit('write', this._address, serviceUuid, characteristicUuid, data[4])
        }
      }
    });
  }
