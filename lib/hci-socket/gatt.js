...
Gatt.prototype.discoverServices = function (uuids) {
  const services = [];
  console.log('gatt.discoverServices')
  const callback = (data) => {
    console.log('gatt.discoverServices callback')
    const opcode = data[0];
    let i = 0;
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      const serviceUuids = [];
      console.log('gatt.discoverServices -- already discovered')
      ...
      console.log('gatt.discoverServices -- 0 read ' + services[services.length - 1].endHandle + ' to 0xffff')
      this._queueCommand(this.readByGroupRequest(services[services.length - 1].endHandle + 1, 0xffff, GATT_PRIM_SVC_UUID), callback);
    }
  };
  console.log('gatt.discoverServices -- 1 read 0x1 to 0xffff')
    ...
    this._queueCommand(this.writeRequest(characteristic.valueHandle, data, false), data => {
      ...
      if (opcode === ATT_OP_WRITE_RESP) {
        this.emit('write', this._address, serviceUuid, characteristicUuid, 0);
          ...
          let errorCode = data[4];
          this.emit('write', this._address, serviceUuid, characteristicUuid, errorCode);
      }
    });
  }
};
