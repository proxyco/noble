...
Gatt.prototype.discoverServices = function (uuids) {
  const services = [];
  debug('gatt.discoverServices')
  const callback = (data) => {
    debug('gatt.discoverServices callback')
    const opcode = data[0];
    let i = 0;
    ...
    if (opcode !== ATT_OP_READ_BY_GROUP_RESP || services[services.length - 1].endHandle === 0xffff) {
      const serviceUuids = [];
      debug('gatt.discoverServices -- already discovered')
      ...
      debug('gatt.discoverServices -- 0 read ' + services[services.length - 1].endHandle + ' to 0xffff')
      this._queueCommand(this.readByGroupRequest(services[services.length - 1].endHandle + 1, 0xffff, GATT_PRIM_SVC_UUID), callback);
    }
  };
  debug('gatt.discoverServices -- 1 read 0x1 to 0xffff')
